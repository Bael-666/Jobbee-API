...
exports.updateJob = catchAsyncErrors(async (req, res, next) => {
    ...
    if (job.user.toString() !== req.user.id && req.user.role !== 'admin'){
        return next(new errorHandler(`User ${req.user.id} is not allowed to update this job.`, 404));
    }
...
exports.deleteJob = catchAsyncErrors(async (req, res, next) => {
    ...
    //Check if user is owner
    if (job.user.toString() !== req.user.id && req.user.role !== 'admin'){
        return next(new errorHandler(`User ${req.user.id} is not allowed to delete this job.`, 404));
    }

    //Deleting files associated with job
    for (let i=0; i < job.applicantsApplied.length; i++){
        let filePath = `${__dirname}/uploads/${job.applicantsApplied[i].resume}`.replace('\\controllers', '');
...
//Searh jobs with radius =>  /api/v1/jobs/:zipcode/:distance
...
exports.applyJob = catchAsyncErrors(async(req, res, next) => {
    ...
    file.mv(`${process.env.UPLOAD_PATH}/${file.name}`, async err => {
        ...
        await Job.findByIdAndUpdate(req.params.id, { $push: { applicantsApplied: { id: req.user.id, resume: file.name }}, 
                                        new: true, 
                                        runValidators: true, 
